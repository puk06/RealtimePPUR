...
internal static class CalculatorUtils
{
    ...
    {
        if (args.Mods.Length == 0) return [.. ruleset.CreateAllMods().Where(m => m is OsuModClassic)];
        var availableMods = ruleset.CreateAllMods();

        IEnumerable<Mod> mods = [];

        foreach (var modString in args.Mods)
        {
    ...
    public static int ManiaScoreCalculator(IBeatmap beatmap, HitsResult hits, string[] mods, int currentScore)
    {
        ...
        double baseScore = 0;
        double bonusScore = 0;
        var (modMultiplier, modDivider) = ModMultiplierModDividerCalculator(mods);
